...
namespace Microsoft.AspNetCore.Authentication
{
    public class AuthenticationSchemeProviderTests
    {
        ...
        [Fact]
        public async Task CanDisableDefaultSignOutFallsbackToSignIn()
        {
            var services = new ServiceCollection().AddOptions().AddAuthenticationCore(o =>
            {
                o.AddScheme<SignInHandler>("signin", "whatever");
        ...
        [Fact]
        public async Task CanDisableDefaultForbidFallsbackToChallenge()
        {
            var services = new ServiceCollection().AddOptions().AddAuthenticationCore(o =>
            {
                o.AddScheme<Handler>("challenge", "whatever");
        ...
        public async Task DefaultSchemesFallbackToOnlyScheme()
        {
            ...
            var provider = services.GetRequiredService<IAuthenticationSchemeProvider>();
            Assert.Equal("single", (await provider.GetAutomaticAuthenticateSchemeAsync()).Name);
        ...
        [Fact]
        public async Task CanDisableDefaultSchemesFallbackToOnlyScheme()
        {
            var services = new ServiceCollection().AddOptions().AddAuthenticationCore(o =>
            {
                o.AddScheme<SignInHandler>("single", "whatever");
                o.EnableDefaultSingleHandlerFallback = false;
            }).BuildServiceProvider();

            var provider = services.GetRequiredService<IAuthenticationSchemeProvider>();
            Assert.Null(await provider.GetAutomaticAuthenticateSchemeAsync());
            Assert.Null(await provider.GetDefaultForbidSchemeAsync());
        ...
        public async Task DefaultSchemesFallbackToAuthenticateScheme()
        {
            ...
            var provider = services.GetRequiredService<IAuthenticationSchemeProvider>();
            Assert.Equal("B", (await provider.GetAutomaticAuthenticateSchemeAsync()).Name);
        ...
        [Fact]
        public async Task CanDisableDefaultSchemesFallbackToAuthenticateScheme()
        {
            var services = new ServiceCollection().AddOptions().AddAuthenticationCore(o =>
            {
                o.EnableDefaultFallback = false;
        ...
        public async Task DefaultSchemesAreSet()
        {
            var services = new ServiceCollection().AddOptions().AddAuthenticationCore(o =>
            {
                ...
                o.AutomaticAuthenticateScheme = "B";
        ...
        public async Task SignInSignOutDefaultsToOnlyOne()
        {
            ...
            Assert.Equal("basic", (await provider.GetAutomaticAuthenticateSchemeAsync()).Name);
        ...
        [Fact]
        public async Task CanDisableSignInSignOutDefaultsToOnlyOne()
        {
            var services = new ServiceCollection().AddOptions().AddAuthenticationCore(o =>
            {
                o.AddScheme<Handler>("basic", "whatever");
            ...
            Assert.Equal("basic", (await provider.GetDefaultAuthenticateSchemeAsync()).Name);
            Assert.Equal("basic", (await provider.GetDefaultChallengeSchemeAsync()).Name);
            Assert.Equal("basic", (await provider.GetDefaultSignInSchemeAsync()).Name);
            Assert.Equal("basic", (await provider.GetDefaultSignOutSchemeAsync()).Name);
        }
